...
def test_real_fixture_hidden_sheet_and_rows_columns(inventory):
    ...
    entry = inventory["fixtures/eval/excel_ja/inputs/JPX-006.xlsx"]
...
def test_real_fixture_stamp_image_covers_cell(inventory):
    """JPX-011.xlsx（実データ）: `assets/廃止スタンプ.png` がセルへ画像アンカーとして実在する
    （A2 起点＝旧売上額/旧税額の行を覆う）。"""
    entry = inventory["fixtures/eval/excel_ja/inputs/JPX-011.xlsx"]
...
def test_real_fixture_covering_text_shapes_with_deprecation_label(inventory):
...
def test_real_fixture_hidden_pptx_slide(inventory):
    """OJA-PPTX-HARD.pptx（実データ）: `p:sld/@show="0"` の非表示スライドが実在する。"""
    entry = inventory["fixtures/eval/office_ja/inputs/OJA-PPTX-HARD.pptx"]
    assert entry["hidden_slides"] == ["ppt/slides/slide2.xml"]


def test_new_fixture_covers_representations_missing_from_real_data(inventory):
