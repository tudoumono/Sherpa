...
def test_default_shows_exactly_three_constructs():
    """既定（`openai_direct_visible` 未設定＝False）では OpenAI 直結を一覧に出さない
    （決定 2026-09-20＝OpenAI 系は Codex(OpenAI) に一本化）。"""
    ids = [c["id"] for c in AC.available_constructs()]
    assert ids == ["ollama_only", "codex_openai", "codex_ollama"]


def test_openai_direct_visible_shows_openai_only_when_provider_matches(monkeypatch):
    """`openai_direct_visible=True` かつ選択中のクラウドプロバイダが openai（既定）のときだけ
    `openai_only` が一覧に加わる。provider が openai でなければ visible=True でも出ない（A7 が
    優先）。"""
    monkeypatch.setattr("sherpa.store.get_system_settings",
                        lambda: {"openai_direct_visible": True})
    ids = [c["id"] for c in AC.available_constructs()]
    assert ids == ["openai_only", "ollama_only", "codex_openai", "codex_ollama"]

    monkeypatch.setattr("sherpa.store.get_system_settings",
                        lambda: {"openai_direct_visible": True, "cloud_provider": "bedrock"})
    ids = [c["id"] for c in AC.available_constructs()]
    assert "openai_only" not in ids

    # 既に読んだスナップショットを渡す経路も同じ判定を使う（DB 再読込に頼らない）。
    ids = [c["id"] for c in AC.available_constructs(system_settings={"openai_direct_visible": True})]
    assert "openai_only" in ids
...
def test_extra_agents_appear_only_when_enabled(monkeypatch):
    ...
    assert [c["id"] for c in AC.available_constructs()] == [
        "ollama_only", "codex_openai", "codex_ollama"]
