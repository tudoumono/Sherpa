...
# ===== coverage_required（網羅性の強化・変更C）=====

def test_review_prompt_coverage_required_adds_enumeration_section():
    """`review_prompt(coverage_required=True)` のときだけ判定の基準へ列挙の網羅の節が足される
    （偽・省略時は従来どおり出ない）。"""
    from sherpa.providers.prompts import review_prompt
    with_coverage = review_prompt("Xごとに Y を教えて", "digest", coverage_required=True)
    without_coverage = review_prompt("Xごとに Y を教えて", "digest", coverage_required=False)
    default_call = review_prompt("Xごとに Y を教えて", "digest")
    assert "【判定の基準＝列挙の網羅】" in with_coverage
    assert "親項目の集合が確定しているか" in with_coverage
    assert "各親項目に子項目が揃っているか" in with_coverage
    assert "列挙に抜けがあるときは「不足」と判定します" in with_coverage
    assert "【判定の基準＝列挙の網羅】" not in without_coverage
    assert "【判定の基準＝列挙の網羅】" not in default_call   # 既定 False は従来どおり


def test_sufficiency_verdict_detects_coverage_requested_from_question():
    """`_sufficiency_verdict` は質問文（`orig_message`）から `investigation_state.
    coverage_requested` を判定し、真なら査読プロンプトへ列挙の網羅の節を足す
    （呼び出し元が明示的に渡す引数は無く、質問文から常時検知する）。"""
    p = _ReviewSynth("sk-dummy", "gpt-5.5", responses=['{"sufficient": true, "missing": ""}'])
    verdict, _nodes, _usage = p._sufficiency_verdict(
        "区分ごとに起動方式を教えて", "qa", "(なし)", "v1", scope_paths=[], layer=None)
    assert verdict == {"verdict": "sufficient", "sufficient": True, "missing": "",
                       "missing_codes": [], "findings": None}
    assert "【判定の基準＝列挙の網羅】" in p._synth_prompts[0]


def test_sufficiency_verdict_omits_coverage_section_for_ordinary_question():
    """網羅要求の語彙を含まない通常の質問では、列挙の網羅の節は付かない。"""
    p = _ReviewSynth("sk-dummy", "gpt-5.5", responses=['{"sufficient": true, "missing": ""}'])
    p._sufficiency_verdict("TAX-RATEは?", "qa", "(なし)", "v1", scope_paths=[], layer=None)
    assert "【判定の基準＝列挙の網羅】" not in p._synth_prompts[0]
...
def _isolate_world_kb(monkeypatch, tmp_path, world: str, files: dict) -> None:
    ...
    `tests/unit/test_agentic_search.py::_isolate_world_kb` と同じ手法。"""
...
def test_scope_evidence_kinds_includes_unconverted_office_original(monkeypatch, tmp_path):
    """未変換（派生MD無し）の Office 原本も実ファイル走査で範囲内に数える——台帳
    （`doc_ledger.documents_for`）は派生MDが無い Office を列挙しないため、これに頼ると
    まだ変換していない Excel 設計書しか無い範囲で `spec_doc` が「該当なし」に誤判定される。"""
    _isolate_world_kb(monkeypatch, tmp_path, "ofw1", {
        "設計/仕様書.xlsx": b"dummy xlsx bytes",   # 派生 .md は無い＝未変換
        "src/PROG1.cbl": "       IDENTIFICATION DIVISION.\n",
    })
    PB._scope_kinds_cache.clear()
    try:
        in_scope, _in_layer = PB._scope_evidence_kinds("ofw1", [], "both")
        assert {"spec_doc", "source", "callgraph"} <= in_scope
    finally:
        PB._scope_kinds_cache.clear()


def test_scope_evidence_kinds_ignores_importance_control_file(monkeypatch, tmp_path):
    """重要度設定ファイル（`_重要度.txt`）は文書ではない——`.txt` として設計書に数えると、ソースだけの
    範囲で満たせない `spec_doc` が必須になり、主張が設計書不足で推定へ格下げされる。"""
    from sherpa.ingest import importance as _imp
    ctrl = f"src/{_imp.IMPORTANCE_CONTROL_FILENAME}" if hasattr(_imp, "IMPORTANCE_CONTROL_FILENAME") else "src/_重要度.txt"
    assert _imp.is_importance_control_path(ctrl)
    _isolate_world_kb(monkeypatch, tmp_path, "impw1", {
        ctrl: "PROG1.cbl 高\n",
        "src/PROG1.cbl": "       IDENTIFICATION DIVISION.\n",
    })
    PB._scope_kinds_cache.clear()
    try:
        in_scope, _in_layer = PB._scope_evidence_kinds("impw1", [], "both")
        assert "spec_doc" not in in_scope
        assert "source" in in_scope
    finally:
        PB._scope_kinds_cache.clear()
...
def test_turn_evidence_kinds_and_claim_kind_gap_propagate_graph_fallback():
    """`_turn_evidence_kinds`/`_claim_kind_gap`（`providers/base.py`）は
    `graph_fallback` をそのまま `investigation_state.evidence_kinds_of`/`claim_evidence_kinds`
    へ伝える——グラフが使えるターン（既定 `graph_fallback=False`）では ripgrep 1件の根拠だけで
    影響調査の必須種別（`source`＋`callgraph`）を満たしたことにしない。グラフが使えないターン
    （`graph_fallback=True`）だけ ripgrep を呼出関係の代替として認める。"""
    from sherpa import investigation_state as IS
    ...
    seen_no_fallback = PB._turn_evidence_kinds(state)
    assert seen_no_fallback == {"source"}
    assert PB._claim_kind_gap(claim, state.evidence, required, seen_no_fallback) == ["callgraph"]

    seen_fallback = PB._turn_evidence_kinds(state, graph_fallback=True)
    assert seen_fallback == {"source", "callgraph"}
    assert PB._claim_kind_gap(claim, state.evidence, required, seen_fallback,
                              graph_fallback=True) == []
...
def test_scope_evidence_kinds_is_cached_between_consecutive_turns(monkeypatch):
    """範囲判定の実ファイル走査は同じ（world・範囲・層）なら短時間だけ再利用する——大規模 world で
    毎ターン全木走査を繰り返さない。判定不能（None）は保持せず次のターンで再試行する。"""
    from sherpa import scope_infer as si
    calls = []
    real = si.safe_files
    monkeypatch.setattr(si, "safe_files",
    ...
    try:
        ...
        assert len(calls) == 2
        # 判定不能（world root 無し）はキャッシュに載らない＝次の呼び出しでも判定し直す
        # （root 自体が無いので実ファイル走査は一度も呼ばれない＝呼び出し回数は増えない）。
