...
def test_safe_claim_preserves_evidence_kinds_when_string_list():
    """S1b: Codex 経路の主張が持つ `evidence_kinds`（文字列 list）は共有経路でも残す。"""
    from sherpa.store.shares import _safe_claim
    out = _safe_claim({"id": "c1", "status": "confirmed", "text": "t",
                       "evidence_refs": ["ev-1"], "reason": "", "reason_code": "",
                       "evidence_kinds": ["source", "callgraph"]})
    assert out["evidence_kinds"] == ["source", "callgraph"]


def test_safe_claim_drops_evidence_kinds_when_not_declared_or_invalid():
    """未申告（`None`・旧6キー形）や型不正は既存 allowlist 方針どおりキーごと落とす。"""
    from sherpa.store.shares import _safe_claim
    out_undeclared = _safe_claim({"id": "c1", "status": "confirmed", "text": "t",
                                  "evidence_refs": ["ev-1"], "reason": "", "reason_code": "",
                                  "evidence_kinds": None})
    assert "evidence_kinds" not in out_undeclared
    out_bad_type = _safe_claim({"id": "c1", "status": "confirmed", "text": "t",
                                "evidence_refs": ["ev-1"], "reason": "", "reason_code": "",
                                "evidence_kinds": "source"})
    assert "evidence_kinds" not in out_bad_type
