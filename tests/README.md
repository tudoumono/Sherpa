...
push・PR のたびに `ci.yml` が `make gate-ci`（ruff＋単体テスト全件＋契約テスト・Postgres service 付き・
壁時計予算チェック込み）を回します。結合テスト・ブラウザ結合テストは CI 化せず、手元で
`make gate-merge`／`make gate-release`（`docs/20-開発ハーネス.md` §5 の段階表）を実行してコマンドと
結果行を記録します。リリース用の workflow は別に `release.yml` があります。

## テスト規範（要約・正典は docs/20）

テストを書く・直す・棚卸しする際の規範は [docs/20-開発ハーネス.md](../docs/20-開発ハーネス.md) が正典です。
ここでは要約だけを示します。

**選ぶ**: 毎回全件を回さず、変更の規模に応じてスイートを選びます。小さな修正は該当するテストか構文チェック
のみ、スライス単位の変更は関連スイート、本流へのマージ前は基本セット（単体テスト全件＋DB 不要の API テスト）
に変更領域の必須スイートを足す、リリース前だけフルゲート（結合・ブラウザ結合・本番前チェック・オフライン
キット検証）を回します。

**新規か既存か**: まず既存のテストを走らせます。既存が赤になったら「契約が変わった（期待値を直す）」か
「回帰した（実装を直す）」かを明示し、どちらか片方だけを直します。新しいテストは、レビュー指摘・不具合・
新しい契約など実害の再現1件につき1本に限って追加し、残す前に「テストを残したまま対象の実装修正だけを戻すと赤・
復元すると緑」を確認します。

**肥大化の抑制**: 単体テスト全件の壁時計時間の上限は5分（確定値）です。超えたら機能追加より先に統合・
削除を優先します。同じ形のテストはコピーせずパラメータ化し、退役した契約のテストはその契約を撤去した
コミットで一緒に消します。

**モックの規律**: モックを許すのは外部境界（LLM API、外部エージェント CLI の偽実行ファイル、ブラウザ側の
API モック、テスト用コーパス）だけです。ファイルは一時ディレクトリ上の実物を使い、データベース・検索
エンジン・グラフストアは結合テストで実物を使います。試験対象そのものの置き換え、内部関数の monkeypatch
による経路の強制、通すためだけの環境変数トグルは禁止します。
