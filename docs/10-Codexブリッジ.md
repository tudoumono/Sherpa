...
**`type` はレンズ＝UC**: `impact`/`troubleshoot`/`qa`/`doc_check` が同じ仕様で型違い（MVP は impact＋薄い troubleshoot/qa）。
...
`GET /documents/{id}/download`。**＋別レンズの薄い probe**（read-only）: `POST /troubleshoot/run`・`POST /qa/run`。
