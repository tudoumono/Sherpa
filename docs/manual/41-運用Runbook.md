...
### S9. 影響調査・原因調べで「検索用データが古い内部形式のままです」と出る

- **症状**: 影響分析やトラブルシュートで、答えの代わりに「この資料フォルダの検索用データが古い内部形式のままです（内部形式が更新されました）。管理者に『今すぐ更新』を依頼してください。」と表示される。
- **原因**: アプリの更新で検索の内部形式（関係グラフの構造）が新しくなった一方、対象フォルダの関係グラフはまだ古い内部形式のまま保存されている（そのフォルダに関係グラフの実データがある場合だけ起きる）。**解析できるファイルの種類を増やした・特定の種類の解析を止めた/再開した・解析する順序を変えた、といった「中身だけ」の更新でもこの内部形式は変わり、同じ現象が起きます**（画面の見た目や機能そのものが変わらないデプロイでも起きうるということです）。
- **影響範囲**: この不一致は登録済みの資料フォルダごとに判定されるため、**デプロイの内容次第では登録しているすべての資料フォルダが対象になり得ます**。デプロイが完了してから、そのフォルダを「今すぐ更新」して再取り込みが終わるまでの間は、そのフォルダに対する影響調査・グラフ表示・トラブルシュートがこの表示のままになります（通常の検索・チャットへの回答自体は止まりません）。
- **復旧**: 管理「資料フォルダ」で該当フォルダの**「今すぐ更新」**を実行する。再取り込みで関係グラフが最新の内部形式で作り直され、自動的に解消する。**データが失われるわけではない**（原本は消えず、通常の再取り込みと同じ扱い）。
- **デプロイ時の運用手順**: 上の「影響範囲」を運用中に広げないため、デプロイ（更新）は次の順序で行います。①メンテナンス時間帯を確保してデプロイを実行する → ②デプロイ完了後、管理「資料フォルダ」に登録している資料フォルダを1つずつ「今すぐ更新」する → ③すべてのフォルダで再取り込みが完了したことを確認する（S7 のロールバック手順と同様、単一のオペレータが1つずつ実行してください）。
- **復旧確認**: 同じ質問をもう一度投げて、通常どおり回答が返ること。
- **コードとして解析する拡張子**: 「関係グラフの構造」を持つのは、専用の解析ロジックに対応した種類のファイルだけです。現時点では COBOL（`.cbl`/`.cob`/`.cobol`）・コピーブック（`.cpy`/`.copybook`）・JCL（`.jcl`）・Java（`.java`）・C（`.c`/`.h`）・C#（`.cs`）・SQL（`.sql`）・設定ファイル（`.properties`／`.yaml`/`.yml`／`.xml`）・画面テンプレート（`.jsp`/`.jspx`/`.jspf`/`.tag`/`.tagx`・画面の目印があるものだけの `.html`/`.htm`/`.xhtml`）・JavaScript（`.js`/`.mjs`）・CSS（`.css`）・シェル/バッチ（`.sh`/`.bash`/`.ksh`/`.zsh`/`.bat`/`.cmd`）・VB（`.vb`/`.bas`/`.cls`/`.frm`/`.ctl`/`.vbs`）が対象です。これ以外の種類のファイルは、通常の検索・出典としては使えますが、関係グラフ（影響調査で経路をたどる仕組み）の対象にはならず、この節の内部形式不一致も起きません。対象の詳細（どの構文からどんな関係を作るか）は [docs/05-グラフ語彙.md](../05-グラフ語彙.md) を参照してください。
- **この告知が出ていなくても「今すぐ更新」で直る食い違い**: 資料そのものは変わっていないのに、関係グラフの中身の一部（語彙の変更など）や Neo4j 側のデータ、全文検索の索引の一部だけが実際の資料と食い違ってしまうことがあります。こうした食い違いも「今すぐ更新」を実行すると自動で検知して作り直します。資料が変わっていない場合、埋め込み（ベクトル検索用の変換）は既存の計算結果を再利用するため、追加の費用は掛かりません。

### S10. Codex の回答が「資料を読めません」「bwrap」のエラーになる／コマンドが全部失敗する

- **症状**: Codex 経路の回答が「資料を読めません」といった内容になる、またはログ（`codex.log`／
  `api.log`）に `bwrap` のエラーが出る。エラーとして表には出ず、ただ回答の精度が低い（本来
  読めるはずの資料を踏まえていない）だけのこともあります。
- **よくある原因**: Ubuntu 23.10 以降の AppArmor ユーザー名前空間制限で、Codex のサンドボックス
  （permission profile・bubblewrap）が起動できず、Codex がコマンドを一切実行できないまま
...
| `codex` | Codex CLI 実行1回ごとの開始/終了サマリ |
...
### 会話ごとの活動記録を数字で見る（make turn-activity）

1 つの会話の各回答について、Codex が何にどれだけ使ったかを**数字だけ**で表示します。回答が遅い・「調べる範囲が広すぎて…」で止まった、といったときに、文脈を何が埋めたかを確かめるのに使います。

```
make turn-activity CONV=136
```

- **会話番号**は `make logs ARGS="codex"` の `conv=…` と同じ番号です。
- **分かること**（回答ごと）: 終了理由とエラー／所要の内訳（準備・Codex・後処理）／実際に効いた設定とアプリの版／調査台帳（完了したか・継続の回数・状態ごとの件数）／本体と下調べ役それぞれの入力（うちキャッシュ）・出力・推論・往復の回数・1 往復の最大入力・圧縮した位置／ツールごとの呼び出し回数・結果の合計量・最大・かかった時間の合計（Sherpa のツールだけ）・切り詰めの回数。
- **出さないもの**: 質問と回答の本文、会話の題名、参照した資料の名前、ツールの引数。これらはデータベースから読みもしません。記録の中の文字列（ツール名・エラーの種類など）も、識別子の形（英小文字・数字・下線）でないものは中身を出さず「（その他）」にまとめます。
- **対象**: 0.13.1 以降の版で、Codex 経路で保存した回答だけです。それより前の回答は「活動記録なし」と表示されます。
- **影響**: 読むだけで、データベースにもアプリの動作にも影響しません。再起動も不要です。
- **置き場所**: 実環境のアプリのディレクトリで最新を取り込んで（`git pull`）から実行するのが簡単です。別の場所に取得したリポジトリから見るときは、そのリポジトリで Python の依存を入れたうえで、`SHERPA_ENV_FILE=<実環境の設定ファイルのパス> make turn-activity CONV=136` のように実環境の設定ファイルを指定します（設定ファイルの中身は表示しません）。
