...
// 質問例（クリックで入力欄に流し込む・自動送信しない）。組み込みの既定文言（`DEFAULT_EXAMPLES`）は
// どれもそのまま送信して意味が通る具体的な質問文で統一する（編集前提の未完成文・会話の続き前提の
// 文は置かない）。4レンズ（影響/トラブル/仕様問い合わせ/資料作成）を一通り示す。
...
// 実際に描画する質問例（既定は組み込み・管理者設定 `chat_examples` があれば差し替える）。
// welcome() の例示ボタン描画と、クリックで入力欄へ流し込む処理の双方（別モジュール）から参照される
// ＝実依存を grep で確認のうえ複数モジュール共有の状態として集約（S4）。`const` だが中身は
// `setChatExamples()` が書き換える（配列参照は不変・要素だけ差し替え＝import した束縛への再代入を避ける）。
export const EXAMPLES = [...DEFAULT_EXAMPLES];

// 管理者設定 `chat_examples`（`GET /settings` の同名フィールド・`sherpa/chat_examples.py` が解決）を
// EXAMPLES へ反映する。`list`: null＝未設定（組み込み既定のまま）／配列（空含む）＝その内容に差し替え
// （空配列＝明示的に非表示）。設定取得に失敗した呼び出し元は本関数を呼ばない＝EXAMPLES は組み込み
// 既定のまま（fail-open）。
