...
const ARM_LABELS = {
  ...
  markitdown: {
    label: '文書を広く読み取り（補助・照合）',
    desc: 'PDF や Office 文書を広く読み取ります。Word / Excel / PowerPoint では標準の読み取り結果と照合して読み落とし（隠れた文字など）を見つけ、文字が少ない PDF を補助的に読み取ります（読み取りソフトは同梱済み）。',
  },
  markitdown_ocr: {
    label: '画像・スキャン文書を AI が見て読み取り（視覚読み取り）',
    desc: '画像やスキャンした文書を AI（視覚モデル）が見て読み取ります。既定はこのパソコンのローカル AI（Ollama）で処理します。クラウド AI（OpenAI）は下の設定で明示的に許可したときだけ使えます（数値は要確認）。',
  },
};
...
const ARM_MISSING_HINT = {
  ...
  markitdown: 'この環境では markitdown が見つかりません（通常は同梱されています。復旧: pip install "markitdown[all]"）。',
  markitdown_ocr: '視覚読み取りに使う AI が使えません。下の「視覚読み取りの AI」設定を確認してください（クラウドを選ぶ場合は許可と OpenAI のキーが必要です）。',
};
