...
### 実環境の固有名（記録に残さない・決定2026-09-24）
- 利用者の実環境（試験環境）に由来する社内固有の名前を、リポジトリ（コード・テスト・文書・RV 台帳）・コミットメッセージ・
  RV への依頼文・メモに**書かない**。対象＝プログラム/ソースのファイル名・画面/帳票名・項目名・設計書名・コード値・
  ホスト名/パス/IP・利用者の質問文そのもの。
- 一般名に置き換える（例: 対象帳票・バッチ側のソース・SQL を持つソース・画面側の定数定義・区分・日付項目・詳細設計書）。
  テストの例文や開発用データ（`fixtures/`）に使う名前も架空のものにする（実環境と無関係な名前＝公開の評価データや
  架空のサンプルは対象外）。評価の結論も一般化して書く。
- 公開リポジトリへ書き出す前に、該当しそうな名前が無いかを確かめる（CLAUDE.md も公開される＝この節にも固有名を書かない）。
