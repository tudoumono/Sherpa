...
### セキュリティ
- メイン推論は外部 OpenAI。「渡す」の可否はこの3行で判定する（**拡張子の許可リストは安全の
  判断根拠ではない**＝解析器の割り当てと種別表示にのみ使う。安全は秘匿判定が担う）:
  - 本文テキストを推論の入力として送るのは**可**。
  - ファイルを外部クラウドへアップロードして永続化するのは**不可**（Files API 不使用）。
  - 秘匿ファイル（`ingest.text_kind.is_sensitive`）の本文を送るのは**不可**。
  区分によるローカル振り分けは将来拡張。
