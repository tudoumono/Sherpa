#!/usr/bin/env bash
# 解析用ログ回収バンドル（`make diag`）の薄いラッパー。運用環境の設定ファイルを doctor.sh と
# 同じ読み方（`sherpa_source_dotenv`・既定 $ROOT/.env・SHERPA_ENV_FILE で差し替え）で取り込んで
# から scripts/collect_diagnostics.py を起動する（読まずに起動すると store が既定の接続先を
# 使い、正常稼働中の環境でもバンドル作成に失敗する）。設定ファイルの中身は端末に出さない。
set -euo pipefail
...
exec "$PYTHON_BIN" "$ROOT/scripts/collect_diagnostics.py" "$@"
