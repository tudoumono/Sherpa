...
def available_constructs(system_settings: dict | None = None) -> list[dict[str, Any]]:
    ...
    `openai_only` は加えて system_settings の `openai_direct_visible`（既定 False・管理画面
    「プロバイダ＋接続先」タブ）が真のときだけ一覧に出す（決定 2026-09-20＝OpenAI 系は
    Codex(OpenAI) に一本化し、直結は既定で利用者の選択肢から外す）。この判定は一覧表示だけの
    制約であり、実行経路・保存値・`construct_id()` は変えない＝既に `agent="openai"` を選んで
    いる利用者の設定はそのまま有効に動く（一覧外として表示される）。
    ...
    `system_settings`（省略可）: 呼び出し側が既に読んだスナップショットを渡すと A7 判定・
    `openai_direct_visible` 判定の両方をそれで行う（省略時は自分で読み、同じスナップショットを
    両方の判定に使う）。
    """
    from sherpa import keys, store
    sys_s = system_settings if system_settings is not None else store.get_system_settings()
