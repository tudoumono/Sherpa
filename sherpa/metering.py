"""チャット以外の LLM 呼び出しの利用量計測。

チャット本回答の usage は引き続き `messages.answer->'usage'` に残る（本モジュールは無関係・二重計上なし）。
未計測だった全 LLM 呼び出し（intent 分類・埋め込み・admin グラフ質問・VLM 視覚読み取り・
rag.md の LLM 成形 等）に、**挙動を一切変えずに** 利用量を記録する。記録は常時 DB に書く（無効化しても
意味がなく、むしろ初回埋め込みコストが計測されない実害があったため、切替は設けていない）。
...
`record()` は DB 記録に加えて `sherpa.usage` ロガー
...
# 'research': `POST /ext/v1/research`（外部 API・チャットを
...
# 'chat-review': メイン査読（清書前の根拠十分性判定・読み直しの限定ツール精読を含む）の
...
# 'rag_render': rag.md の LLM 成形（取り込み後にバックグラウンドで後追い実行する
...
_MAX_STR_FIELD_LEN = 256   # metering.record の防御的長さ上限（下記 docstring 参照）


def _clamp_str(v, limit: int = _MAX_STR_FIELD_LEN):
    """`v` を文字列化して `limit` 字で切り詰める。`None` は `None` のまま
...
def record(kind, provider, model, usage, *, user_id=None, world=None, calls=1,
    ...
    `elapsed_ms`: 省略時は `acc_begin()`/`acc_end()`
    ...
    （巨大文字列の反復複製によるストレージ増幅対策）。

    `connect_timeout`/`statement_timeout_ms`（両方省略可・既定 None＝無期限）は `add_usage_event()`
    の INSERT へそのまま転送する（本関数の DB 接続は常に1回のみ）。

    DB 記録に成功したら `sherpa.usage` ロガーへも INFO 1行を出す（`log_usage_line`）。
...
def log_usage_line(kind, provider, model, tokens: dict, calls, world, elapsed: float | None,
    ...
    """`sherpa.usage` ロガーへの INFO 1行。`record()` が成功パスから呼ぶほか、
...
@contextmanager
def suppress():
    """このスレッドの `record()` を一時的に無効化する。

    読み取り専用契約の経路が、計測有効時でも `usage_events` へ書かないためのガード
    （汎用の抑制プリミティブ）。計装済みの下位関数が内部で
...
def acc_begin() -> None:
    ...
    呼び出し元は必ず try/finally で対にする。`t0` は `acc_end()` がこの
...
def acc_add(usage) -> None:
    ...
    `suppress()` 中は外側の既存スコープにも**合算しない**（読み取り専用の入れ子呼び出しから、
    別 kind の usage が混入して記録される合成可能性の穴を塞ぐ）。
    """
    try:
...
def acc_end() -> tuple:
    ...
    既存の返り値契約は不変（拡張しない）——pop したフレームの経過秒は `acc_elapsed()`
...
def acc_elapsed() -> float | None:
    """直近の `acc_end()` が pop したフレームの経過秒。1回読んだら消費する
