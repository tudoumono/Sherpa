...
_TURNS_MULT = {"quick": 1, "standard": 1, "deep": 2, "max": 3}
# grep/ES ヒット上限・読み取り窓（`run_tool`/`run_qa`）は同じ倍率を共有する。
_RATIO_MULT = {"quick": 1.0, "standard": 1.0, "deep": 1.5, "max": 2.0}
...
def scaled_turns(base_max_turns: int, profile) -> int:
    """反復上限（`Main Round 上限`相当・`MAX_TURNS`）。クイック/標準=×1・深く=×2・最大=×3（切り捨て）。"""
...
def codex_reasoning_for(base_reasoning: str, profile) -> str:
    """Codex 推論レベル。**深さでは変えない**——どの深さでも管理画面の基準値
    （`codex_reasoning`／`depth_base_codex_reasoning`）をそのまま使う。

    深さが効くのは探索量（`scaled_turns`/`scaled_ratio`/`scaled_depth`）と査読の巡数
    （`review_rounds_for`）だけで、推論の強さは設定で固定する。深さの妥当性検証（fail-loud）は
    ここでも通す（呼び出し元が未検証の値を渡していないことをこの経路でも保証する）。
    """
