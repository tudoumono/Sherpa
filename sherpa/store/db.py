...
_SCHEMA = [
    ...
    # 別物で、対応する user 発言は user_message_id で引く。activity_json は検証済みの activity
    # （§3.1 の契約により本文・資料名・ツール引数を含まない）を丸ごと保持する。
    ...
    # 1 ターン×エージェント番号（activity.agents の配列添字）×ツール 1 行。activity の無い
    # （旧版で保存された）ターンは行を持たない——ツール別の内訳は activity にしか無い情報のため。
