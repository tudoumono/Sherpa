"""モデルのコンテキスト窓（`docs/proposals/2026-09-02-RAG表現の全形式展開と文脈保持.md` §3.4・
2026-09-03 裁定で導入・`docs/proposals/2026-09-22-Codex経路の精度・網羅性と費用の改善.md` で
管理画面登録値（段1）を撤去）。

モデル名→窓 tokens の3段解決（プロバイダAPI > コード同梱シード表 > 不明）を持つ。ツール結果
バイト予算（`agentic_search.py::resolve_tool_result_budgets`）はもうこの解決結果を使わない
（利用者裁定「AI が持つ文脈窓を Sherpa が制限しない」で min() 方式ごと撤去済み）。
...
def query_ollama_context_length(base_url: str, model: str, *,
    ...
    """Ollama `/api/show` の `context_length` をライブ照会（段1）。
...
# ---- コード同梱シード表（段2・OpenAI/Azure 主要モデル） --------------------------------------

# Azure OpenAI は同じ基盤モデルを配信するため同じ値を流用する（provider="openai" で共用・Azure の
# デプロイ名がモデル名と異なる場合はここで解決できず「不明」へ落ちる）。値は各モデルの公表
# コンテキスト窓（出典: OpenAI 公式モデル一覧 platform.openai.com/docs/models）。**知らない値は
# でっち上げない**——本アプリのモデルカタログ組み込み既定に含まれる "gpt-5.5"/"gpt-5.4-mini" 等は
# 非公開/将来のモデル名のため意図的に未収載（unknown が正しい挙動）。
...
def seed_window_tokens(provider: str, model: str) -> int | None:
    """コード同梱シード表（段2）。OpenAI/Azure（provider="openai"）のみ対象（他プロバイダは
...
def resolve_window_tokens(provider: str, model: str, *, system_settings: dict | None = None,
    ...
    """モデル窓サイズ（tokens）の3段解決:
    プロバイダAPI（Ollama `/api/show`・Anthropic Models API） > シード表（OpenAI/Azure） > 不明。

    戻り値 `(tokens|None, source)`。`source` は `"api"`/`"seed"`/`"unknown"`。
    `provider`/`model` が空なら即 `(None, "unknown")`。`ollama_base_url`（provider="ollama" の
    ときのみ意味を持つ）・`anthropic_client`（`.models.retrieve()` を持つクライアント）は段1の
    照会に使う——どちらも省略すればその段は単にスキップされ、次の段へ進む（fail-safe）。
    `system_settings` は段1（Ollama）の SSRF allowlist 判定にのみ使う
    （`query_ollama_context_length`→`llm.ollama_url` 参照）。
    """
