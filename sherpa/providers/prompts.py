...
（Codex への KB パス案内）・`_PLAIN_PROMPT*`（ナレッジ参照オフ時の素プロンプト）・
...
# ナレッジ参照オフのときの素のプロンプト（検索結果＝事実を渡さない＝出典なしの一般回答）。
_PLAIN_PROMPT = ("あなたは親切な日本語アシスタントです。社内ナレッジ（資料）は参照していません。"
                 "一般的な知識の範囲で簡潔に答えてください。**出典・社内資料・ファイル名・引用に基づくとは言わない**。"
                 "資料に基づく確認が必要なら『ナレッジ参照をオンにしてください』と促す。\n\n【質問】{q}\n回答:")

# HIGH-1 fix: 個人ファイルのヒットを plain プロンプトに注入するテンプレート。
_PLAIN_PROMPT_WITH_PERSONAL = (
    "あなたは親切な日本語アシスタントです。社内ナレッジ（資料）は参照していませんが、"
